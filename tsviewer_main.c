                           ...
                           int max_value, int *out)
{
    ...
    if (digits == 0) return FALSE_INT;
    if (value < min_value || value > max_value) return FALSE_INT;
...
/* The prebuilt Windows GTK3 bundle used by the cross-compilation target in the
   Makefile ships 3.24 DLLs alongside 3.10 headers, so a few functions the DLLs
